...
RETURNS TABLE(nome TEXT, ponto INTEGER, vitoria INTEGER, empate INTEGER, derrota INTEGER) AS $$
    ...
    DECLARE
        ...
        nomeTime CURSOR(cod TEXT) FOR SELECT sigla FROM time_ WHERE sigla IN (SELECT time1 FROM jogo WHERE campeonato = cod) OR 
                                                          sigla IN (SELECT time2 FROM jogo WHERE campeonato = cod);
		
    BEGIN
        CREATE TEMPORARY TABLE classif(nome TEXT, ponto INTEGER, vitoria INTEGER, empate INTEGER, derrota INTEGER);

        FOR iniciais IN nomeTime(codigo) LOOP
            SELECT COUNT(*) FROM jogo WHERE (time1 = iniciais.sigla AND gols1 > gols2) OR (time2 = iniciais.sigla and gols2 > gols1)
            INTO vitoria;

            SELECT COUNT(*) FROM jogo WHERE (time1 = iniciais.sigla OR time2 = iniciais.sigla) AND gols1 = gols2
            INTO empate;
            ...
            INSERT INTO classif VALUES(nome, ponto, vitoria, empate, derrota);
        END LOOP;

	RETURN QUERY SELECT * FROM classif ORDER BY(classif.ponto, classif.vitoria) DESC LIMIT pos2 OFFSET pos1;
